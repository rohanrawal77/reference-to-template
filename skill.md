...
- **Snapshot `data-x`/`data-y` at load time and diff against that snapshot, never against a hardcoded `0`.** Once a pasted Copy Changes diff bakes a non-zero position into the HTML as the new default, comparing against `0` makes that baked-in default report as a "change" on every future load with zero edits made. Follow the same load-time-snapshot pattern already used for color/font-size/tracking.
